...
export class BgCenterRegion extends LitElement {
  ...
  render() {
    return html`
      <div class="bg-img theme-primary" style="--img: url('/img/${this.img.src}-400w.jpeg')">
        <div class="cover ${this.overrides} cover-nav">
          <div class="principle inset region prose stack align-center text-center gap-xs">
            ${this.badge ? html` <a href="/" class="badge badge-primary">${this.badge}</a> ` : ''}
            <h1>${this.headline}</h1>
            <p>${this.body}</p>
            ${this.btn
              ? html` <a href="${this.btn.href}" class="btn ${this.btn.variant || ''}"> ${this.btn.label} </a> `
              : ''}
          </div>
        </div>
      </div>
